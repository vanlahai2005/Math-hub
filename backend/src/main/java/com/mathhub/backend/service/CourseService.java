...
public class CourseService {
    ...
    public List<Course> getAllCourses() {
        ...
        if (courses.isEmpty() || containsBrokenEncoding(courses)) {
    ...
    private boolean containsBrokenEncoding(List<Course> courses) {
        return courses.stream()
                .map(Course::getTitle)
                .filter(title -> title != null)
                .anyMatch(title -> title.contains("Ã") || title.contains("á»") || title.contains("Ä"));
    }
